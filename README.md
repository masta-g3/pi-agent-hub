...
The top line names the current surface as `FLEET`, `WORKFLOW`, `PINNED FLEET`, or `WORKSPACE`, then shows owner-tree, pin, `needs you`, and health signals that fit. With a filter active, tree counts use visible/total form. `▸` marks a collapsed owner with child rows; press `→` to expand it to `▾`, and `←` to collapse it again.
