...
export async function ensureMultiRepoWorkspace(session: ManagedSession, env: NodeJS.ProcessEnv = process.env): Promise<ManagedSession> {
  ...
  await writeWorkspaceContextFile(workspaceCwd, projectPaths, linkNames);
...
async function writeWorkspaceContextFile(workspaceCwd: string, projectPaths: string[], linkNames: string[]): Promise<void> {
  const entries: Array<{ repoPath: string; linkName: string; contextPath: string; content: string }> = [];
  for (let i = 0; i < projectPaths.length; i += 1) {
    const context = await readProjectContextFile(projectPaths[i]!);
    if (!context) continue;
    entries.push({ repoPath: projectPaths[i]!, linkName: linkNames[i]!, contextPath: context.path, content: context.content });
  }
  if (!entries.length) return;
  await writeFile(join(workspaceCwd, "AGENTS.md"), renderWorkspaceContextFile(entries), "utf8");
}

async function readProjectContextFile(projectPath: string): Promise<{ path: string; content: string } | undefined> {
...
function renderWorkspaceContextFile(entries: Array<{ repoPath: string; linkName: string; contextPath: string; content: string }>): string {
  return [
    "# Multi-repo workspace instructions",
    "",
    "Generated by pi-agent-hub for this multi-repo workspace. Source repositories are not modified.",
    "",
    ...entries.flatMap((entry) => [
      `## Repository: ${entry.linkName}`,
      "",
      `Path: ${entry.repoPath}`,
      `Workspace link: ${entry.linkName}`,
      `Source context: ${entry.contextPath}`,
      "",
