...
export class SessionsController {
  ...
  async refresh(now = Date.now()): Promise<void> {
    ...
    for (const [id, observation] of observations) {
      ...
      if (appliedObservationIds.has(id)) {
        ...
        const interaction = observation.presence === "present" && isFreshHeartbeat(observation.heartbeat, now) ? observation.heartbeat.interaction : undefined;
  ...
  private sessionsWithMetadata(): RuntimeSession[] {
    return this.registry.sessions.map((session) => {
      ...
      return interaction || context || workflow !== session.workflow || statusEvidence
        ? { ...session, ...(interaction ? { interaction } : {}), ...(context ? { context } : {}), workflow, ...(statusEvidence ? { statusEvidence } : {}) }
        : session;
    });
  }
}
