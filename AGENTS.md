...
- Keep one selected-session action workspace with positive-only conditional content in this order: identity, explicit request, real task text, plain workflow position, exceptional Hub guidance, enabled catalog actions, then optional appended evidence. Missing categories contribute no rows, and the first policy-ordered command carries the primary-action marker. Prefer a complete ticket subtitle for task text and use its description only when no subtitle exists. Keep one action label and one exact command target per actionable row, including the pinned decision strip. Use the same renderer at every width: 34 cells at 120–159, 44 at 160+, and an exact-session full-width screen below 120 without a second mode header. Short-height compaction must preserve identity, the primary action, and requested evidence. The first narrow `Enter`/double-click opens it without side effects; workspace `Enter` runs Open/Restart. Never capture, render, or search raw tmux pane tails or Pi conversation content.
...
- Optional producer-neutral `pi-agent-hub-context` Pi entries carry bounded ticket context and producer-accepted attention through the Hub heartbeat. Structurally validate the latest versioned snapshot, ignore unknown fields, and never persist context into `registry.json` or couple it to Rules. Display attention only for waiting/idle rows; do not infer or reclassify it from confidence, stage, liveness, or workflow state. Fresh delivery requires a producer request ID; never infer an ID or persist delivery history.
