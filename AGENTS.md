...
- Emit runtime and cockpit-placement evidence from the existing reducers, keep it on transient runtime/render projections, and never persist or couple it to workflow state. See `docs/STRUCTURE.md` for the observation, invalidation, CLI, and narrow Info contracts.
...
- Keep catalog-owned direct keys, `:` palette entries, dashboard Help, configured actions, and stable footer controls sourced from `src/tui/dashboard-commands.ts`. All invocation paths must honor the same availability guard. Bind contextual command and dialog/picker submission to the exact session/project identity; palette session results only select and ephemerally reveal, never attach, restart, or acknowledge. See `docs/STRUCTURE.md` for search, ordering, precedence, and reveal contracts.
