...
- Optional `kind: "subagent"` registry rows are owned by `pi-tmux-subagents`: keep them nested and short in the fleet, keep task text in the action workspace/filtering, and disable normal session lifecycle/group/order actions on them.
