...
Compaction is separate from mode and workflow state. Its bounded heartbeat `operation` has `kind: "compact"`, an `id`, and a `phase` of `running`, `complete`, `error`, or `cancelled`, with an optional bounded `error`. Terminal operation evidence remains until new work or compaction; the dashboard exposes it only with fresh, non-shutdown liveness and confirmed tmux presence. Generic operations never enter the registry. Fork readiness instead uses durable `forkPreparation` control, described in [Structure](STRUCTURE.md).
