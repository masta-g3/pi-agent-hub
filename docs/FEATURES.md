...
| Action workspace | Select a session; use `Enter` below 120 columns | Read positive task context, plain workflow position, exceptional guidance, and enabled actions without reading raw pane output. |
...
When the dashboard is 120 columns or wider, `Enter` switches the current tmux client to the selected live session and briefly shows the equivalent `tmux switch-client -t <session>` command. For explicit questions, the same command is labeled **Answer** and first checks for an exact existing pin. Below 120 columns, the first `Enter` opens the action workspace and the second runs that same command. On a stopped or error session, the action restarts it instead of attempting to attach. Opening a `waiting` session marks it read before attaching, so it can show `idle` after you return; merely opening the workspace does not acknowledge it, and `a` remains the manual mark-read shortcut.
