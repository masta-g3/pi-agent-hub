...
| `Alt+Q` or `Ctrl+Q` | Return from a focused live pane to the sidebar |
| `/` | Filter sessions |
...
The project top line summarizes visible sessions plus nonzero `needs you` and `health` tree counts; the workflow board retains runtime status counts. Card richness and responsive width choices are derived per render and are never persisted. Press `?` for the full help/legend and `i` for live evidence in the selected session's action workspace.
