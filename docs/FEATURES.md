...
| Action workspace | Select a session; use `Enter` below 120 columns | Read the explicit request, maintainer recommendation, enabled actions, bounded context, state, workflow, and tree without reading raw pane output. |
...
| `Enter` | Below 120 columns, open the action workspace first; inside it or at 120+, open/switch or restart the selected session |
| `1`–`4` | Assign, replace, move, or swap a fixed quadrant panel; press the selected session's current number again to close it |
...
| `A` | Archive the selected session and close its panel if shown |
...
The action workspace answers three questions in order: what explicitly needs the maintainer, what the maintainer should do next, and which enabled commands can do it. Bounded producer context, runtime state, workflow position, and tree context follow. At 120+ columns it is persistent on the right. Below 120 columns, the first `Enter` or session-row double-click opens an exact-session full-width workspace without attaching, restarting, or acknowledging; `Enter` inside runs Open/Restart and `Escape` returns to the fleet. Action clicks use the same exact-target catalog dispatcher as direct keys and `:`. Disabled commands stay discoverable only in the palette.

Press `i` to toggle live evidence inside that same workspace. The evidence reports one current observation in causal order: tmux, heartbeat, read state, runtime result plus placement, then independent workflow provenance. Synthetic Archived/disclosure targets cannot open stale session information. `pi-hub explain <exact-id-or-unique-prefix>` prints the same semantic evidence after one read-only full-fleet observation; it does not write registry state or provide status history. Raw pane output and Pi conversation content are never rendered or searched.
...
Sessions running the optional `workflow-runtime` extension also show a workflow rail: the compact current-position marker and short code (`◉EX` or `✓EX`) in project rows and the producer-defined full rail (`✓ PL─◉ EX─· RV─· RF─· CM · ticket`) in the action workspace. Project rows retain the title first, then reveal `backlog`, group, workflow short, and activity age as width permits; running rows omit the redundant age, and Archived rows retain their time-since-archive label. Junction cards use the dense marker-only form (`✓◉···`) before producer activity. When activity is absent, deterministic plan progress uses the adaptive `▰`/`▱` phase grid, flattening and then reducing to a ten-cell ratio plus count only as width requires. The runtime owns the ordered ids, short codes, friendly labels, and optional current-position completion; Hub does not mirror a workflow vocabulary.
...
When Rules focus mode is active, its producer-owned display metadata substitutes `FOC` for the active `EX` short in project rows and action-workspace rails. The card stays in the `EXECUTE` lane and reserves `FOC` at its right edge. Focus is display-only in Hub: it adds no lane, ordering priority, control, or animation. Stopped sessions retain their Execute lane but fall back to `EX` and omit stale focus detail.
...
When the dashboard is 120 columns or wider, `Enter` switches the current tmux client to the selected live session and briefly shows the equivalent `tmux switch-client -t <session>` command. Below 120 columns, the first `Enter` opens the action workspace and the second runs that same command. On a stopped or error session, the action restarts it instead of attempting to attach. Opening a `waiting` session marks it read before attaching, so it can show `idle` after you return; merely opening the workspace does not acknowledge it, and `a` remains the manual mark-read shortcut.

### Sidebar workspace

Use `1`–`4` in the dashboard to place the selected live session in a fixed screen quadrant: `1` is top-left, `2` top-right, `3` bottom-left, and `4` bottom-right. Empty slots may be assigned in any order and holes remain stable. A bare digit assigns an empty quadrant, replaces its session, or moves or swaps an already shown session. Pressing the selected session's current quadrant again closes that panel. Every result leaves keyboard focus in the sidebar; `x` then `1`–`4` remains an explicit close path.
